...
const NRLDCCharges = () => {
  ...
  const congestionData = useMemo(() => {
    const filtered = data.filter(item => {
      // Filter based on EA_TYPE = 'N' 
      const isCongestionType = item[1] === 'N' || 
                               (item[4] && item[4].toLowerCase().includes('StoaBills'));
  ...
  return (
    ...
    <div className="bg-gradient-to-r from-gray-900 to-blue-900 text-white p-4 text-center relative overflow-hidden">
      ...
      <h1 className="text-3xl font-bold mb-1 relative z-10">NRLDC Charges Bill</h1>
    </div>
