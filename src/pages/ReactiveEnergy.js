...
const ReactiveEnergy = () => {
  ...
  const congestionData = useMemo(() => {
    const filtered = data.filter(item => {
      // Filter based on EA_TYPE = 'R' 
      const isCongestionType = item[1] === 'R' || 
                               (item[4] && item[4].toLowerCase().includes('reactiveEnergy'));
  ...
  return (
    ...
    <div className="bg-gradient-to-r from-gray-900 to-blue-900 text-white p-4 text-center relative overflow-hidden">
      ...
      <h1 className="text-3xl font-bold mb-1 relative z-10">Reactive Energy Account</h1>
    </div>
